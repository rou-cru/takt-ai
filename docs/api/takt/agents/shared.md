...
ValidateSubAgentBase validates the fields common to all target sub\-agent requests: ID, description, instructions, and model assignment. ValidateSubAgentBase validates the required fields shared by sub\-agent definitions and applies target\-specific model validation. It returns the first validation error encountered.
...
RenderGlobalPrompt returns the native global prompt artifact for the given RenderGlobalPrompt creates a global prompt artifact with the specified path and content. It requires non\-empty content and ensures the artifact content ends with a newline.
