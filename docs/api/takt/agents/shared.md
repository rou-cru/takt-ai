...
ValidateSubAgentBase validates the fields common to all target sub\-agent requests: ID, description, instructions, and model assignment. The caller passes a target\-specific model validation function.
...
RenderGlobalPrompt returns the native global prompt artifact for the given target. The path and label are target\-specific.
